...
def legendary_among_best_attackers() -> None:

    '''percentage of legendary poekmons among best attackers'''

    attack = poki.sort_values(['Attack'], ascending=[0]).head(50)
    legos = attack.loc[attack['Legendary'] == True].count()[-1]
    non_legos = 50 - legos 

    fig = plt.subplots()
    plt.title(
              'Percentage of legendary Pokemons among 50 best attackers', 
              fontsize=18, 
              fontweight='bold'
             )
...
legendary_among_best_attackers()